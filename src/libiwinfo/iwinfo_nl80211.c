...
static int nl80211_get_assoclist_cb(struct nl_msg *msg, void *arg)
{
	...
	static struct nla_policy stats_policy[NL80211_STA_INFO_MAX + 1] = {
		...
		[NL80211_STA_INFO_TX_BITRATE]    = { .type = NLA_NESTED },
		[NL80211_STA_INFO_SIGNAL]        = { .type = NLA_U8     },
		[NL80211_STA_INFO_CONNECTED_TIME]= { .type = NLA_U32    },  //wangyaoyang : for reciving queue len
		[NL80211_STA_INFO_TX_FAILED]     = { .type = NLA_U32    },  //wangyaoyang : for reciving queue len
	};
	...
	if (attr[NL80211_ATTR_STA_INFO] &&
	    !nla_parse_nested(sinfo, NL80211_STA_INFO_MAX,
	                      attr[NL80211_ATTR_STA_INFO], stats_policy))
	{
                if (sinfo[NL80211_STA_INFO_TX_FAILED])                                 //wangyaoyang
			e->tx_failed = nla_get_u32(sinfo[NL80211_STA_INFO_TX_FAILED]); //wangyaoyang

                if (sinfo[NL80211_STA_INFO_CONNECTED_TIME])                                 //wangyaoyang
			e->t_connect = nla_get_u32(sinfo[NL80211_STA_INFO_CONNECTED_TIME]); //wangyaoyang
