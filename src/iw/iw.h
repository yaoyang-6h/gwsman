...
#define MAX_ASSOC_ENTRIES           5
typedef struct __mpath_info {
    unsigned char   m_mac_next_hop[ETH_ALEN];
    unsigned char   m_mac_dest_add[ETH_ALEN];
    int             m_sn;
    unsigned int    m_metric;
    int             m_qlen;
    unsigned int    m_exptime;
    unsigned int    m_discovery_timeout;
    unsigned char   m_retries;
    unsigned char   m_flag;
    char            m_signal;
    char            m_noise;
    unsigned short  m_mcs_tx;
    unsigned short  m_mcs_rx;
} MPATH_INFO;

typedef struct __mpath_tab {
    unsigned char   m_station_no;
    unsigned int    m_ip_address;
    unsigned char   m_mac_local[ETH_ALEN];
    int             m_channel;
    int             m_txpower;
    int             m_temperature;
    int             m_nEntries;
    MPATH_INFO      m_entry[MAX_ASSOC_ENTRIES];
} MPATH_TABLE;

extern MPATH_TABLE curr_mpath_table;
