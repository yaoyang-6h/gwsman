...
int iw_debug = 0;
int shm_identifier = 0;     //wangyaoyang
void* shm_memory = NULL;    //wangyaoyang
...
int gws_mpath_init(struct nl80211_state* state) {
    cmd_size = abs((long)&__section_set - (long)&__section_get);
    return nl80211_init(state);
}

void gws_mpath_exit(struct nl80211_state* state) {
    nl80211_cleanup(state);
}

int gws_mpath_dump(struct nl80211_state* nlstate,char* ifname) {
    const struct cmd *cmd_mpath_dump = NULL;
    char*  argv_scan[3];
    ...
    argv_scan[2] = "dump";
    mpath_table_clear();
    return __handle_cmd(nlstate, II_NETDEV, 3, argv_scan, &cmd_mpath_dump);
}

int gws_mpath_new(struct nl80211_state* nlstate,char* ifname,char* dst,char* next_hop) {
    const struct cmd *cmd_mpath_new = NULL;
    char*  argv_mpath_new[6];

    argv_mpath_new[0] = ifname;
    argv_mpath_new[1] = "mpath";
    argv_mpath_new[2] = "new";
    argv_mpath_new[3] = dst;
    argv_mpath_new[4] = "next_hop";
    argv_mpath_new[5] = next_hop;
    
    return __handle_cmd(nlstate, II_NETDEV, 6, argv_mpath_new, &cmd_mpath_new);
}

int gws_mpath_set(struct nl80211_state* nlstate,char* ifname,char* dst,char* next_hop) {
    const struct cmd *cmd_mpath_set = NULL;
    char*  argv_mpath_set[6];

    argv_mpath_set[0] = ifname;
    argv_mpath_set[1] = "mpath";
    argv_mpath_set[2] = "set";
    argv_mpath_set[3] = dst;
    argv_mpath_set[4] = "next_hop";
    argv_mpath_set[5] = next_hop;
    
    return __handle_cmd(nlstate, II_NETDEV, 6, argv_mpath_set, &cmd_mpath_set);
}